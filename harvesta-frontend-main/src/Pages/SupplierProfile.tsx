...
import { Star, Calendar, Building2, Leaf, Image, MessageSquare } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs defaultValue="overview" className="space-y-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-card rounded-xl p-6 border">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {supplier.productionMethods.map(method => (
                ...
                <div className="bg-card rounded-xl p-6 border">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{supplier.yearsInOperation} years</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Annual Capacity</span>
                      <span className="font-medium">{supplier.capacity}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Total Sales</span>
                      <span className="font-medium">{supplier.totalSales.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              ...
              <div key={review.id} className="bg-card rounded-xl p-6 border">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
                    {review.buyerAvatar ? (
                      <img src={review.buyerAvatar} alt="" className="w-full h-full rounded-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1">
                    ...
                    {review.productName && (
                      <p className="text-sm text-muted-foreground mb-2">Purchased: {review.productName}</p>
                    )}
          ...
          <TabsContent value="activity" className="space-y-4">
            {supplier.activityPosts.map(post => (
              <div key={post.id} className="bg-card rounded-xl p-6 border">
                <Badge variant="outline" className="mb-3">{post.type.replace('_', ' ')}</Badge>
                <h3 className="font-semibold mb-2">{post.title}</h3>
                <p className="text-muted-foreground mb-3">{post.content}</p>
                {post.image && (
                  <img src={post.image} alt="" className="rounded-lg w-full max-w-md" />
                )}
