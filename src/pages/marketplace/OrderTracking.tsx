...
import { mockOrderTracking } from '@/lib/mockData';
              ...
              <div key={vendor.vendorId} className="flex items-center justify-between p-4 rounded-lg bg-muted/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{vendor.carrier} {vendor.trackingNumber && `• ${vendor.trackingNumber}`}</p>
                  </div>
                </div>
                <div className="text-right">
                  <Badge variant={vendor.status === 'Shipped' ? 'default' : 'secondary'}>{vendor.status}</Badge>
                  <p className="text-sm text-muted-foreground mt-1">Est. {new Date(vendor.estimatedDelivery).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </CardContent>
