...
function DisputeCard({ dispute }: { dispute: Dispute }) {
  ...
  const statusColors = {
    ...
    investigating: 'bg-blue-100 text-blue-700 border-blue-300 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-700',
    escalated: 'bg-red-100 text-red-700 border-red-300 dark:bg-red-900/30 dark:text-red-400 dark:border-red-700',
    resolved: 'bg-green-100 text-green-700 border-green-300 dark:bg-green-900/30 dark:text-green-400 dark:border-green-700',
  ...
  return (
    <motion.div variants={itemVariants} layout>
      <Card className={cn('overflow-hidden border-l-4 transition-all', dispute.status === 'escalated' ? 'border-l-red-500' : dispute.status === 'open' ? 'border-l-yellow-500' : dispute.status === 'investigating' ? 'border-l-blue-500' : 'border-l-green-500')}>
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Open', value: stats.open, icon: Clock, color: 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400' },
          { label: 'Escalated', value: stats.escalated, icon: AlertTriangle, color: 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' },
          { label: 'Resolved', value: stats.resolved, icon: CheckCircle2, color: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' },
        ].map((stat) => (
