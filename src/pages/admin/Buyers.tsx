...
const mockBuyers: Buyer[] = [
  ...
  { id: 'b5', name: 'Ahmed Hassan', email: 'ahmed@dubaitrade.ae', phone: '+97150123456', company: 'Dubai Trade Co', type: 'b2b', region: 'Middle East', totalOrders: 67, totalSpent: 120000000, trustScore: 45, status: 'blocked', lastOrder: new Date(Date.now() - 2592000000).toISOString() },
];
...
function BuyerCard({ buyer }: { buyer: Buyer }) {
  ...
  const statusColors = {
    ...
    flagged: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400',
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3 pt-3 border-t">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    <Star className={cn('h-3 w-3', buyer.trustScore >= 80 ? 'text-yellow-500 fill-yellow-500' : 'text-muted-foreground')} />
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'B2B Accounts', value: stats.b2b, icon: ShoppingBag, color: 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' },
          { label: 'B2C Customers', value: stats.b2c, icon: Users, color: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400' },
          { label: 'Flagged', value: stats.flagged, icon: AlertTriangle, color: 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400' },
        ].map((stat) => (
