...
import { fetchDashboardStats, fetchLiveFeed, DashboardStats, LiveFeedItem } from '@/lib/admin-api';
...
function KPICard({ title, value, subtitle, trend, icon: Icon, iconColor, delay = 0 }: {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
    >
      <Card className="cursor-pointer overflow-hidden relative group">
            ...
            <div className="space-y-1 sm:space-y-2">
              ...
              {trend !== undefined && (
                <motion.div 
                  ...
                  className={cn('flex items-center gap-1 text-xs sm:text-sm', trend >= 0 ? 'text-green-600' : 'text-red-600')}
                >
...
function LiveFeedCard({ items }: { items: LiveFeedItem[] }) {
  ...
  const getSeverityColor = (severity?: string) => {
    const colors: Record<string, string> = { success: 'text-green-600', warning: 'text-yellow-600', error: 'text-red-600', info: 'text-blue-600' };
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <KPICard title="Total Orders" value={formatNumber(stats.orders.total)} subtitle={`${stats.orders.pending} pending`} icon={ShoppingCart} iconColor="bg-orange-500" delay={1} />
        <KPICard title="Active Sellers" value={formatNumber(stats.users.activeSellers)} subtitle={`${stats.users.newToday} new today`} icon={Users} iconColor="bg-blue-500" delay={2} />
        <KPICard title="In Transit" value={formatNumber(stats.logistics.inTransit)} subtitle={`${stats.logistics.exceptions} exceptions`} icon={Truck} iconColor="bg-purple-500" delay={3} />
      </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                {[
                  { label: 'Pending', value: stats.orders.pending, icon: Clock, color: 'text-yellow-600 bg-yellow-100' },
                  { label: 'Shipped', value: stats.orders.shipped, icon: Package, color: 'text-blue-600 bg-blue-100' },
                  { label: 'Delayed', value: stats.orders.delayed, icon: AlertTriangle, color: 'text-red-600 bg-red-100' },
                  { label: 'Cancelled', value: stats.orders.cancelled, icon: XCircle, color: 'text-gray-600 bg-gray-100' },
                ].map((item, i) => (
                  <motion.div
                    key={item.label}
          ...
          <Card className="h-full">
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-3 sm:gap-4 text-center">
                {[
                  { label: 'Open', value: stats.disputes.open, color: 'bg-yellow-100 text-yellow-600' },
                  { label: 'Escalated', value: stats.disputes.escalated, color: 'bg-red-100 text-red-600' },
                  { label: 'Resolved', value: stats.disputes.resolved, color: 'bg-green-100 text-green-600' },
                ].map((item, i) => (
                  <motion.div
                    key={item.label}
