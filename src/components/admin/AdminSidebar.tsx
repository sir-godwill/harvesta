  ...
  return (
    ...
    >
      ...
      <div className="flex h-16 items-center justify-between border-b border-border px-4">
        ...
        <Button
          ...
          className={cn(
            ...
            isCollapsed && 'absolute -right-3 top-4 z-50 rounded-full bg-accent shadow-md'
          )}
      ...
      <ScrollArea className="h-[calc(100vh-4rem)]">
                ...
                <CollapsibleContent className="ml-4 space-y-1 pt-1">
                  {item.children?.map((child) => (
                    <NavLink
                      ...
                      to={child.href}
                      className={cn(
                        'flex items-center rounded-lg px-3 py-2 pl-6 text-sm transition-colors',
                        isActive(child.href)
        ...
        {/* Region Selector */}
        {!isCollapsed && (
          <div className="mx-3 mt-4 rounded-lg border border-border bg-accent/50 p-3">
            <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
              <Globe className="h-3.5 w-3.5" />
              <span>Active Region</span>
            </div>
            <select className="w-full rounded-md border-0 bg-accent px-2 py-1.5 text-sm text-foreground focus:ring-1 focus:ring-ring">
