...
export function ProductCard({
  ...
  const lowestPrice = product.pricingTiers.reduce(
    (min, tier) => (tier.pricePerUnit < min ? tier.pricePerUnit : min),
  ...
  return (
    <div
      className={cn(
        "group bg-card rounded-xl border border-border overflow-hidden hover:shadow-lg transition-shadow",
        className,
      )}
    >
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-secondary font-medium">
            As low as ${(lowestPrice ?? 0).toFixed(2)} for bulk orders
          </p>
        </div>
        ...
        <div className="flex gap-2 pt-2">
          <Button size="sm" className="flex-1 gap-1.5" onClick={onAddToCart}>
