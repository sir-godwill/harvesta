  ...
  return <div className="flex gap-1.5 overflow-x-auto scrollbar-hide pb-1 snap-x snap-mandatory min-w-0">
      ...
      <div className="bg-gradient-to-br from-orange-50 to-rose-50 rounded-lg p-1.5 border border-orange-100 min-w-[45%] max-w-[45%] flex-shrink-0 snap-start h-[100px]">
        ...
        <div className="grid grid-cols-2 gap-1">
          {deals.map(deal => <Link key={deal.id} to={`/product/${deal.id}`} className="group">
              ...
              <p className="price-primary font-semibold truncate text-xs">{formatPrice(deal.price)}</p>
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-1.5 min-w-[45%] max-w-[45%] flex-shrink-0 snap-start h-[100px]">
        ...
        <div className="grid grid-cols-2 gap-1">
          {superDeals.map((deal, index) => <Link key={deal.id} to={`/product/${deal.id}`} className="group">
              <div className="relative w-8 h-8 rounded overflow-hidden bg-white/10 mb-0.5">
                <img src={deal.image} alt={deal.name} className="w-full h-full object-cover transition-transform group-active:scale-105" loading="eager" />
                {index === 0 && <span className="absolute bottom-0 left-0 bg-primary/90 text-primary-foreground text-[5px] px-0.5 rounded-tr">
                    AI
                  </span>}
              </div>
              <p className="text-primary font-semibold truncate text-xs">{formatPrice(deal.price)}</p>
      ...
      {/* View All Link */}
      <Link to="/products" className="bg-gradient-to-br from-secondary/10 to-secondary/5 rounded-lg p-1.5 border border-secondary/20 min-w-[15%] flex-shrink-0 snap-start flex flex-col items-center justify-center gap-0.5 h-[100px]">
